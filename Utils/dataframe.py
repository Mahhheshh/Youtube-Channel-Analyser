...
class Cleanup:
    ...
    def apply_int(self) -> None:
        self.df[["Viewcount", "Likecount", "Commentcount"]] = self.df[["Viewcount", "Likecount", "Commentcount"]].applymap(self.to_int)
    
    def dates_cleanup(self) -> None:
        #taking published date only 
        self.df["Publishedat"] = self.df["Publishedat"].apply(lambda string: string.split()[0])
    ...
    def to_sec(slef, time_string: str) -> float|int|None:
        try:
            ...
            return timedelta(hours=val.hour ,minutes=val.minute, seconds=val.second).total_seconds()
        except: 
            pass
        try:
    ...
    def calculate_like_percent(self) -> None:
        self.df["Likepercentage"] = ((self.df["Likecount"]/self.df["Viewcount"])*100).round(2)
    
    def calculate_comment_percent(self)-> None:
        self.df["Commentpercentage"] = ((self.df["Commentcount"]/self.df["Viewcount"])*100).round(2)
    
    def clean(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        self.df = pd.DataFrame(dataframe)
        self.df.columns = self.df.columns.str.capitalize()
        self.df.fillna(value={"Viewcount":0, "Likecount":0, "Commentcount":0}, inplace=True)
        self.apply_int()
        self.apply_sec()
        self.dates_cleanup()
...
class Visualize(Cleanup): 
    order = {
    "Uploadmonth": ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"],
    "Uploadday": ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
    }
    ...
    def last_fifty_uploads(self):
        fig = px.area(
            ...
            markers="True", labels={"Publishedat":"Month"}, 
            range_x=[]
        )
        return fig
    ...
    def views_per_year(self, year:int):
        if year not in self.years:
            year = self.years[-1]
        data = self.df[self.df["Uploadyear"]==year].groupby("Uploadmonth", as_index=False).agg({"Viewcount":"sum"})
        fig = px.bar(
            data, 
            x="Uploadmonth", 
            y="Viewcount", 
    ...
    def top_ten_videos(self):
        data = self.df.nlargest(10, "Viewcount").sort_values("Viewcount").loc[:, ["Title", "Viewcount", "Likecount"]]
        fig = px.bar(
            data,
